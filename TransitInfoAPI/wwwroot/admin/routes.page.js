...
function renderRoutes(list) {
  ...
  const rows = list.map(r => `<tr>
    ...
    <td><code class="small">${esc(r.feedId||'-')}</code></td>
    <td><code class="small">${esc(r.operatorGlobalId||'-')}</code></td>
    <td><code>${esc(r.onestopId)}</code></td>
  </tr>`).join('');
  document.getElementById('content').innerHTML = `<div class="table-responsive"><table class="table table-striped table-hover"><thead class="table-dark"><tr><th>Name</th><th>Short Name</th><th>Type</th><th>Operator</th><th>Feed</th><th>Operator ID</th><th>Global ID</th></tr></thead><tbody>${rows}</tbody></table></div><small class="text-muted">${list.length} route(s) shown</small>`;
}
