// Admin editing map. Restored from the interactive map that used to live at /map/ (removed in
// 06678a2 when /map/ became the passenger trip planner). This copy lives under /admin/, is gated by
...
// Surface base-map failures instead of showing a silent white page. The basemap style, glyphs and
// tiles come from https://tiles.openfreemap.org; if that host is unreachable (offline, a firewall,
// or a content blocker) MapLibre fires 'error' and nothing paints. Report it rather than leaving the
// operator staring at a blank map. Tile-level errors can be frequent, so only report once.
let basemapErrorShown = false;
map.on('error', (e) => {
  if (basemapErrorShown) return;
  basemapErrorShown = true;
  const msg = (e && e.error && e.error.message) || '';
  // Persistent, not the 8s auto-hide showMapError: a missing basemap does not fix itself, and a
  // banner that has already vanished is exactly the "just white, no explanation" state this avoids.
  const el = document.getElementById('mapError');
  if (el) {
    el.textContent = 'Base map failed to load — tiles.openfreemap.org is unreachable from this browser (offline, a firewall, or a content/ad blocker). ' + msg;
    el.classList.remove('d-none');
  }
});
...
// moved to. Abort the previous batch before starting a new one.
...
map.on('load', () => {
  ...
  // Route lines colored by type (solid for normal, dashed if manually edited)
  map.addLayer({
    id: 'routes-line',
    type: 'line',
    source: routesSource,
    filter: ['!=', ['get', 'shapeEdited'], true],
    paint: {
      'line-color': [
        'match', ['get', 'routeType'],
        'Tram', '#1f78b4',
...
map.on('mouseenter', 'routes-line-edited', () => { map.getCanvas().style.cursor = 'pointer'; });
map.on('mouseleave', 'routes-line-edited', () => { map.getCanvas().style.cursor = ''; });

map.on('click', 'routes-line', (e) => {
  const props = e.features[0].properties;
  showRouteDetails(props);
});
map.on('click', 'routes-line-edited', (e) => {
  const props = e.features[0].properties;
  showRouteDetails(props);
});
...
function esc(s) {
  // Escapes quotes as well as angle brackets, so esc() output is safe inside quoted HTML attributes
  // (station/route names go into href and style attributes below) as well as text nodes.
...
function showRouteDetails(props) {
  ...
  const edited = props.shapeEdited ? ' (edited)' : '';
...
function showMobilityStationSidebar(props) {
  ...
  let html = `
    ...
    <div class="info-row"><strong>Station ID</strong> <code>${esc(props.stationId || '-')}</code></div>
