// Same-origin, through GetThereAPI's map proxy. The page used to be handed TransitInfoAPI's address
// as an ?api= parameter and call it directly, which broke the one-way rule (the client is supposed
// to talk only to GetThereAPI) and needed credentials this page never had.
const API_BASE = '/api/map/upstream';

// The proxy is authenticated. A WebView navigation cannot carry an Authorization header, so the host
// pushes the token in after load via window.setAuthToken(...); requests queue until it arrives.
// Deliberately not passed in the URL — that would leak it into server logs and history.
let authToken = null;
let authReady = null;
let resolveAuthReady = null;

if (window.self !== window.top) {
  // Embedded without a host to inject a token: nothing to wait for.
...
// Browsing the page directly (an admin checking the map) already has a session in sessionStorage.
if (!authToken) {
  const stored = sessionStorage.getItem('auth_token');
  if (stored) window.setAuthToken(stored);
}

/** fetch() against the proxy, waiting for the token and attaching it. */
...
function loadMapData() {
  ...
  const stationUrl = API_BASE + `/stations?format=geojson&lat=${center.lat}&lon=${center.lng}&radiusKm=${radiusKm}`;
  const routeUrl = API_BASE + `/routes?format=geojson&minLat=${sw.lat}&minLon=${sw.lng}&maxLat=${ne.lat}&maxLon=${ne.lng}`;
  const mobilityUrl = API_BASE + `/mobility/stations?format=geojson&lat=${center.lat}&lon=${center.lng}&radiusKm=${radiusKm}`;

  apiFetch(stationUrl).then(data => {
  ...
  apiFetch(mobilityUrl).then(data => {
...
function loadVehicles() {
  apiFetch(API_BASE + '/realtime/vehicles').then(resp => {
    const vehicles = resp || [];
    const fc = {
...
function showStationDetails(id, props) {
  ...
  apiFetch(API_BASE + `/stations/${id}/departures?count=5`).then(resp => {
    const deps = resp || [];
    if (deps.length === 0) {
...
// ── Mode filter ──────────────────────────────────────────────────────────────
// The MAUI map screen draws its transport-mode chips natively over this page (frame 3a of the
// design) and calls in here when one is toggled. Modes are the chip keys; an empty or absent
// list means "show everything", which is also the state the page loads in.
...
window.setModeFilter = function (modes) {
  ...
  const showBikes = showAll || active.includes('Bikes');
  ...
  // aggregate across types and cannot be filtered by a member property, so they are hidden while
  // a transport filter is on and the individual stops carry the answer instead.
  const wanted = active.flatMap(m => MODE_ROUTE_TYPES[m] || []);
...
// ── Native map controls ──────────────────────────────────────────────────────
// The two round buttons in frame 3a. Both are no-ops until the style has loaded, which is why the
// host guards its calls with `window.x && window.x()`.
...
window.toggleRouteLines = function () {
